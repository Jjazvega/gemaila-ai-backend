...
class AuthUseCases:
    ...
    async def register(self, email: str, password: str, tenant_id: str) -> str:
        existing_user = await self.user_repository.get_by_email(email=email, tenant_id=tenant_id)
    ...
    async def me(self, user_id: int, tenant_id: str):
        user = await self.user_repository.get_by_id(user_id=user_id, tenant_id=tenant_id)
        if user is None:
